...
uint256 hashGenesisBlock("0x891164e2350c9fb5143499265a4d4f0d33b314e6935e1f83ec4b8a084956c392");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Boondollar: starting difficulty is 1 / 2^12
...
{
    // Boondollar: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 420000 * COIN;

    // Subsidy is cut in half every 840000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 100000); // Boondollar: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = .5 * 24 * 60 * 60; // Boondollar: .5 days
static const int64 nTargetSpacing = 60; // Boondollar: 1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Boondollar: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Boondollar: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Boondollar: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "01/14/2014 - The Gigapause is still driving us nuts.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x1d16583a2e050b08d508528e50b93e033b324981c42d7bb0c5d0ae9802ae6a82"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Boondollar: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BoondollarMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BoondollarMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BoondollarMiner(CWallet *pwallet)
{
    printf("BoondollarMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Boondollar-miner");
    ...
    try { loop {
        ...
        printf("Running BoondollarMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BoondollarMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BoondollarMiner, pwallet));
}
